...
(function () {
  ...
  function init(elOrId: HTMLElement|string, options: Options) {
    try {
      ...
      console.log('OFACalculator: Initializing oia-form (form-only) embed in container:', elOrId);
      ...
      const src = new URL(`${opts.iframeBase!.replace(/\/$/, '')}/embed/oia-form`);
      ...
      console.log('OFACalculator: oia-form iframe created and message listener attached');
    } catch (error) {
      console.error('OFACalculator: Error initializing oia-form:', error);
      throw error;
    }
  }
  window.OFACalculator = { init };
  console.log('OFACalculator: oia-form loader script loaded');
})();

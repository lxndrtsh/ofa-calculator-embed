...
function postToParent(msg) { window.parent.postMessage(msg, '*'); }
function useBoot() {
...
export default function CommunityPage() {
    ...
    const handleLookup = async () => {
        if (!boot || lookupLoading || !form.state || !form.county || form.county === 'County Not Listed')
            return;
    ...
    return (<div ref={rootRef} style={{ padding: 16, maxWidth: 720, margin: '0 auto', position: 'relative' }}>
      {/* Dev Debug Panel */}
      <button type="button" onClick={() => setShowDev(!showDev)} style={{
            position: 'absolute',
            top: 16,
            right: 16,
            padding: '4px 8px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '32px', position: 'relative' }}>
        {[
            ...
            return (<div key={num} style={{ display: 'flex', alignItems: 'center', flex: isLast ? '0 0 auto' : '1' }}>
              <div onClick={() => handleStepClick(num)} style={{
                    display: 'flex',
      ...
      {step === 1 && (<div style={{ display: 'grid', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
            ...
            }} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '6px', fontSize: '14px', width: '100%' }}>
              ...
              <label style={{ flex: 1 }}>
                ...
                <select value={form.county} onChange={e => setForm({ ...form, county: e.target.value })} disabled={!form.state} required style={{
                padding: '10px',
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            <label style={{ flex: 1 }}>
              ...
              <input type="number" min={1} value={form.population} onChange={e => setForm({ ...form, population: e.target.value })} required placeholder="Auto-populated or enter manually"/>
            </label>
            <button type="button" onClick={handleLookup} disabled={!form.state || !form.county || form.county === 'County Not Listed' || lookupLoading} style={{
                padding: '10px 14px',
                border: 0,
                borderRadius: 8,
                background: '#666',
      ...
      {step === 2 && (<div style={{ display: 'grid', gap: 16 }}>
          ...
          <label>
            <span>Phone</span>
            <input type="tel" value={form.phone} onChange={e => handlePhoneChange(e.target.value)} placeholder="(123) 456-7890" maxLength={14}/>
          </label>
          <label>
      ...
      {step === 3 && (<div style={{ display: 'grid', gap: 16 }}>
          ...
          {submitting && (<div style={{ textAlign: 'center', padding: '40px 20px' }}>
      ...
      {/* Navigation Buttons */}
      {step < 3 && (<div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: step === 1 ? 'flex-end' : 'space-between' }}>
          {step > 1 && (<button type="button" onClick={handleBack} style={{
                    padding: '10px 20px',
                    border: '1px solid #ccc',
          ...
          <button type="button" onClick={handleNext} disabled={(step === 1 && !validateStep1()) ||
                (step === 2 && !validateStep2())} style={{
                padding: '10px 20px',
                border: 0,
                borderRadius: 8,
                background: '#111',
                color: 'white',
                cursor: ((step === 1 && !validateStep1()) ||
                    (step === 2 && !validateStep2())) ? 'not-allowed' : 'pointer',
                opacity: ((step === 1 && !validateStep1()) ||
                    (step === 2 && !validateStep2())) ? 0.5 : 1
            }}>
            {step === 2 ? 'Submit' : 'Next'}
          </button>
        </div>)}
